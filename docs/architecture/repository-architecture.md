...
9. Web apps MUST NOT import `crypto-local`, `storage-sqlite`, `service`, `api-server`, `host-local`, private protocol artifacts, or server-only runtime modules. Dashboard access is through `api-client` only.
