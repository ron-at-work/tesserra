...
TESSERRA is a project-defined evidence-verification layer, not a replacement for identity, PKI, OAuth/OIDC, MCP, A2A, SPIFFE/SPIRE, authorization servers, policy engines, or provenance standards. The following boundary is intentional:

| Function                                                      | Owner                                               | TESSERRA action                                                                                                                    |
| ------------------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
...
| Verify a bounded chain of project evidence                    | TESSERRA project-defined RFC under local policy     | Parse, verify signatures and source references, check typed bindings/attenuation/status/replay, and return a deterministic result. |

## Protocol-neutral concepts proposed for TESSERRA
...
| Runtime evidence reference           | Tesserraer/issuer, workload identifier, credential/key reference, verification time, trust-domain context                                 | Proof of delegation, user consent, or a logical-agent identity.                                                  |
...
3. carry an **additional**, versioned TESSERRA proof only in a documented MCP extension/binding mechanism;
...
- replace MCP OAuth tokens with TESSERRA evidence;
...
- use authorization-server metadata as an TESSERRA trust-anchor discovery mechanism.
...
SPIFFE federation is not authorization/delegation federation. SVID rotation or workload re-attestation affects runtime-evidence freshness; it neither revokes nor creates an TESSERRA authority chain by itself.
...
A future adapter may negotiate a separate extension containing an TESSERRA evidence reference/proof and bind it to the A2A task/message/artifact digest plus intended audience/resource. It must preserve A2A's own authentication scheme and task lifecycle. It must explicitly report unsupported negotiation, stripping, or verification failure instead of treating an ordinary A2A success as an TESSERRA-verified action.
